...
app.post("/checkout", function (req, res) {
	...
	deviceData: req.body.device_data
		...
		if (result.success) {
			...
			sa.post("https://ua-acm-web-util.herokuapp.com/member/payforsemester").send({purchaseID: result.transaction.id, size: shirtSize, email: userEmail, datePaid: result.transaction.createdAt.substring(0,9), paymentType: result.transaction.paymentInstrumentType}).end(function(err, res) {
			});
			res.send("ok");
		}
		else {
